...
public:
	...
	virtual void render(const std::string& SubTexture, SDL_Point destRect, std::optional<std::string> textureID = std::nullopt);
	virtual void renderEx(const std::string& SubTexture, SDL_Point destPos, std::optional<std::string> textureID = std::nullopt, float angle = 0.0f, SDL_RendererFlip flip = SDL_RendererFlip::SDL_FLIP_NONE);
